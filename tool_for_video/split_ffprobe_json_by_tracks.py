...
def main(d: dict):
    ...
    for stream in d.get("packets", []):
        ...
        if packets:
            ...
            write_json_file(
                file_name_base + file_name_suffix + ".json", {"packets": packets}
            )
    ...
    for stream in d.get("frames", []):
        ...
        if frames:
            file_name_suffix = f"_{stream_type}_{stream_index}"
            write_json_file(
                file_name_base + file_name_suffix + ".json", {"frames": frames}
            )
    p_stream_index.clear()
    ...
    for stream in d.get("packets_and_frames", []):
        ...
        packets_and_frames = [
            i
            for i in d.get("packets_and_frames", [])
            if i["stream_index"] == stream_index
        ]
