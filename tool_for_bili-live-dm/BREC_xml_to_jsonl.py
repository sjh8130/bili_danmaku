...
def process_brec_xml_to_jsonl(file_path):
	...
	with open("Z:\\test.json", "a", encoding="utf-8") as w:
		for dm in dmks:
			...
			w.writelines(str(dm_info[0][4])+json.dumps({"cmd": "DANMU_MSG", "info": dm_info}, ensure_ascii=False, separators=(",", ":"))+"\n")
			# print(i.attrib["raw"])
			# b = 1
		for gift in gifts:
			gift_data = json.loads(gift.attrib["raw"])
			w.writelines(str(int(gift_data["timestamp"]*1000))+json.dumps({"cmd": "SEND_GIFT", "data": gift_data}, ensure_ascii=False, separators=(",", ":"))+"\n")
			# print(i.attrib["raw"])
			# b = 1
