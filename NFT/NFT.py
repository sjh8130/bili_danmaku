...
if __name__ == "__main__":
	...
	ps = 100
	# total = (int(sys.argv[2])/ps).__ceil__()
	...
	while (mark):
		i += 1
		pn = i
		data = downloader(pn, item_id, ps)
		response_d = json.loads(data)["data"]
		if data == a:
			print(f"[NFT]{item_id}:未找到该收藏品")
			mark = False
			not_found = True
			break
		elif data == b"":
			print(f"[NFT]{item_id}_{i}:403 服务器正在休息")
			continue
		else:
			item_name = response_d["item_name"]
			issuer_name = response_d["issuer_name"]
			w_d = json.dumps(response_d, ensure_ascii=False)
			open(f"NFT_{item_id}.json", "ab").write(bytes(w_d, encoding="utf-8")+b"\x0d\x0a")
		if i >= (response_d["total"]/ps).__ceil__():
			mark = False
	if not not_found:
		open("NFT_ID.txt", "a", 4096, "utf-8").write(f"{item_id}\t{issuer_name}\t{item_name}\n")
	session.close()
