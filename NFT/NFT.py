...
c = b'{"code":12006047,"message":"12006047","ttl":1,"data":null}'
...
def get_data(item_id: int | str, base_path: str):
    ps = 100
    run = True
    not_found = False
    pn = 1
    while run:
        ...
        if pn >= float(response_d["total"] / ps).__ceil__():
            run = False
    if not not_found:
...
def main(base_path: str):
    ...
    for item_id in range(2330, 10000):
        if os.path.exists(base_path + f"\\NFT_{item_id}.json"):
            continue
        get_data(item_id, base_path)
