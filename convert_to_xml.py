...
for i in range((len(jsonData["elems"]))):
    ...
    item = "\t<d p=\"" + str(format(progress/1000, ".5f")) + "," + str(jsonData["elems"][i]["mode"]) + "," + str(jsonData["elems"][i]["fontsize"]) + "," + str(jsonData["elems"][i]["color"]) + "," + str(jsonData["elems"][i]["ctime"]) + ","+"0"+"," + jsonData["elems"][i]["midHash"] + "," + jsonData["elems"][i]["id"] + "," + str(jsonData["elems"][i]["weight"]) + "\">" + str(content).replace("&", "&amp;").replace(">", "&gt;").replace("<", "&lt;") + "</d>\n"
    XML_items += item
